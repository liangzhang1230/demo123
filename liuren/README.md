...
## 可操作 / 录入面（数值权归老板 · 改动即时重算）

不是只读报告——每一屏都能录入、编辑、即时重算：

- **员工档案**：新增 / 编辑 / 停用（Salesperson 全字段）。
- **治理体检输入**：编辑 SII/EI/AHC 的原始输入（日报/点名/抽检/审批/查看/处方/异议/建议/确认/忽略/达成/履约/拦截/棘轮），四指数随录随算。
- **系数总表**：全部标量系数可改（写 coefOverride）+「恢复出厂」；判定即时生效。
- **价签输入**：月均毛利/招聘周期/回本/爬坡当量/加薪/缩短比例/年招聘量 → 75万/67.7%/20.8×/42.6万 随之重算。
- **产权 M28**：新建带教/使用费协议；irrevocable「尝试下调」只留痕。
- **前程合约**：六模板拼装；irrevocable 覆盖计入 AHC。
- **M30 双通道**：销售端「我有异议 / 我有个建议（可匿名）」真正写入，老板端「采纳/维持/重核·忽略」处理，EI 分母随之变动。
- **分红三重闸 + 池比例 + 蓝图里程碑**：可编辑、可勾选达成。
- **发放预检 / 离职登记向导**：选人 + 录数 → 三态判定 / 余震名单 + 交接卡。
- **信封 / 授权**：导出、导入他方信封、粘贴续期码。
