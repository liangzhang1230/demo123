...
(() => {
  ...
  function vInput() {
    ...
    <div class="grid g2" style="margin-top:8px">
      ${h.card('去年的底子', `<div class="frm">
        ...
        ${h.field('招一个人多久能到岗', h.seg('company.hiringCycleDays', [{ v: 15, t: '半个月' }, { v: 45, t: '一个半月' }, { v: 90, t: '三个月' }], c.hiringCycleDays, 'num'), '从发招聘到人坐到工位')}
      </div>`)}
      ${h.card('团队怎么干活', `<div class="frm">
        ${h.field('团队怎么协作', h.seg('company.complementLevel', ENUMS.complementLevel.map(([v, t]) => ({ v, t })), c.complementLevel), '决定「团队奖该不该发」')}
    ...
    <div class="grid g2">
      ...
      ${h.card('毛利率 + 人', `<div class="frm">
        ...
        ${h.field(`现有销售 / 主管 ${h.linked()}`, `<b style="font-size:16px">${salesN} 个销售 · ${mgrN} 个主管</b>`, '由员工档案实时统计，不用手填')}
    ...
    <div data-act="dj.more" style="cursor:pointer;user-select:none;display:flex;align-items:center;gap:8px;margin-top:12px">
      ...
      <span class="hint">去年人均回款 · 去年走了几个人 · 招人要多久 · 团队怎么协作</span>
    </div>
    ${moreOpen ? moreBody : ''}
  ...
  function tierTableHtml() {
    ...
    const rowCells = (lv, key, idx, label) => {
      ...
      return `
        ...
        <td class="num mono">${rc != null ? '回款的 ' + fmt.pct(rc) : DASH}</td>
  ...
  function cardAHtml(R) {
    ...
    body += killer('这不是工资单，是一张「普通人干出头部水平」压强表——提成越高、底薪越低，越像鱼饵。');
    body += h.src('📎 回算逻辑：参考底薪=城市基数×层级倍数×周期因子；达标月收入查阈值矩阵；提成=(达标收入×90%−底薪)÷月回款目标。自定义值以你为准。');
    return dcard('A', '账单A · 这样定薪亏不亏', `好招指数 ${A.goodHire.toFixed(2)} · 销售${GRADE_SHORT[R.inp.tierGrade]}·管理层${GRADE_SHORT[R.inp.mgrGrade]}档`, s.rc != null ? '回款的 ' + fmt.pct(s.rc) : (s.r != null ? fmt.pct(s.r) : DASH), '销售提成', body);
  }
  function cardBHtml(R) {
    ...
    return dcard('B', '账单B · 该招几个人', `拍脑袋 ${B.S1} 人 → 算完账 ${B.S6} 人`, `${B.S6}`, '真实需招人数', body);
  }
  function cardCHtml(R) {
    ...
    return dcard('C', '账单C · 走一个人亏多少钱', '简版（3 项）', fmt.wan(C.tag), '走一个销售亏（明细口径）', body);
  }
  function cardDHtml(R) {
    ...
    return dcard('D', '账单D · 该配几个主管', `需 ${D.managerNeeded} 主管 · 缺 ${D.gap}`, `${D.gap}`, '主管缺口', body);
  }
  function cardEHtml(R) {
    ...
    return dcard('E', '账单E · 团队奖该不该发', `TIS ${E.TIS.toFixed(0)}`, `${dot} ${E.TIS.toFixed(0)}`, '团队激励适配分', body);
  }
